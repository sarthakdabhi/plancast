...
## Local writing-model selection (0.4.0 prerelease)

Interactive `setup-local` lists the three curated Qwen3 models, download sizes, and testing status. Enter retains the current choice. `--model <id>` skips the picker and overrides the environment for setup; `--yes` accepts the selected/default model. An explicit `PLANCAST_LOCAL_SCRIPT_MODEL` also skips the picker. Non-interactive setup requires one of these explicit options before downloading. Successful setup atomically saves the choice to `settings.json` under Plancast application support; failed or interrupted setup preserves the old preference. Generation resolves environment override, saved preference, then Qwen3 14B. Cloud configuration does not read local preferences. No API keys are persisted.

## Content-based conversation framing (0.5.0 prerelease)

`--framing auto|plan|document` defaults to auto. Extraction kind no longer selects prompt semantics. Shared evidence and conversation prompts (dialogue-v9, dialogue-v9-llama-v1) adapt to the source content for every provider. Legacy evidence category names remain for compatibility, but are explicitly defined as topic/context, central ideas/findings/events/proposal, support, sequence, and limitations. Only source-supported fields are populated; explicit pending actions are retained, not inferred from narrative events. Optional framing biases presentation without permitting invention.

The composer requests four bounded question slots (null for inactive groups), tailored to each group's evidence, then alternates questions with cited explanatory passages. It reserves question words in the budget and validates question shape before TTS. The local sentence-pair transform applies only to passages, not questions. Final source-grounding, coverage, total budget, and duration checks remain. Framing is included in preflight and the generation manifest. Prompt/fixture tests are not semantic accuracy guarantees; review real model transcripts as part of validation.
